...
static inline long tt_dev_unlocked_ioctl(struct file *fp, unsigned int cmd, unsigned long arg) {
    ...
    int uncopied_byte_cnt, direction, buf_len, i;
    char *buffer;
    ...
    switch (direction) {
        ...
        // TODO (tnballo): add signalling here to test Process -> Kernel Driver -> Other Process with indirection
        // switch on cmd
    ...
    for (i = 0; i < buf_len; ++i) {
        dev_info(tt_dev, "0x%02x ", (char)buffer[i]);
    }
    dev_info(tt_dev, "\n");
    */
