...
static inline int tt_dev_generic_open(struct inode *inode, struct file *file) {
	...
	dev_info(tt_dev, " opened device \'%s\' (major %d, minor %d). Open count = %d, reference count = %d\n",
		TT_DEV_NAME,
		imajor(inode),
		iminor(inode),
...
static inline int tt_dev_generic_release(struct inode *inode, struct file *file) {
	...
	dev_info(tt_dev, " closed device \'%s\' (major %d, minor %d). Close count = %d, reference count = %d\n",
		TT_DEV_NAME,
		imajor(inode),
		iminor(inode),
		(int)module_refcount(THIS_MODULE),
...
static int __init tt_mdev_generic_init(void) {
	...
	if (misc_register(&tt_mdev)) {
		...
		pr_err("Failed to register device \'%s\' (major %d, minor %d)\n",
			TT_DEV_NAME,
			tt_mdev.major,
			tt_mdev.minor
		);
	...
	dev_info(tt_dev, " successfully registered device \'%s\' (major %d, minor %d)\n",
		TT_DEV_NAME,
		tt_mdev.major,
		tt_mdev.minor
	);
...
static void __exit tt_mdev_generic_exit(void) {
	...
	dev_info(tt_dev, " unregistering device \'%s\' (major %d, minor %d)\n",
		TT_DEV_NAME,
		tt_mdev.major,
		tt_mdev.minor
	);
